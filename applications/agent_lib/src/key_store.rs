...
impl KeyStore {
        ...
        for line in lines {
            ...
            let key = match scheme {
                b"ECDSA_NISTP256_SHA256" => {
                    let private_key = ecdsa::p256::PrivateKey::try_from(key_bytes.as_slice()).map_err(|_| Error::Encoding)?;
                    let ecdsa_key = EcDsaP256PrivateKey::new(private_key, DigestAlgorithm::Sha256);
                    store.add_ecdsa_p256_key(ecdsa_key).map(|(id, key)| (id, PublicKey::EcDsaP256Key(key)))
                }
                b"ED_25519" => {
                    let secret_scalar: [u8; 32] = key_bytes.as_slice().try_into().map_err(|_| Error::Encoding)?;
                    let private_key = Ed25519PrivateKey::new(ed25519::SigningKey::from_bytes(secret_scalar));
                    store.add_ed25519_key(private_key).map(|(id, key)| (id, PublicKey::Ed25519Key(key)))
                }
    ...
    pub fn sign_for_ecdsa_p256_id(&self, id: [u8; 32], message: &[u8], rng: &mut impl CryptoRng) -> Result<EcDsaP256Signature, Error> {
    ...
    pub fn sign_for_ed25519_id(&self, id: [u8; 32], message: &[u8]) -> Result<Ed25519Signature, Error> {
        let entries = self.entries.read().map_err(|_| Error::Signing)?;
        match entries.get(&id).ok_or(Error::UnknownID)?.get_key() {
