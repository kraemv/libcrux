...
impl<'a, Scheme, const NONCE_LEN: usize, const TAG_LEN: usize> AeadEncryptRequest<'a, Scheme, NONCE_LEN, TAG_LEN> {
    pub fn new(id: ID, plaintext: &'a [u8], nonce:[u8; NONCE_LEN] , aad: &'a [u8]) -> Self {
        Self { id, nonce, aad, plaintext, scheme: PhantomData }
    }
}
...
impl<'a, Scheme, const NONCE_LEN: usize, const TAG_LEN: usize> TryFrom<&'a mut [u8]> for AeadEncryptRequest<'a, Scheme, NONCE_LEN, TAG_LEN> {
    ...
    fn try_from(request: &'a mut [u8]) -> Result<Self, Self::Error> {
        let (plaintext_len, request_slice) = usize::try_read_from_prefix(request)
        ...
        let (id, request_slice) = ID::try_read_from_prefix(request_slice)
            .map_err(|_| Error::MalformedRequest)?;

        let (nonce, aad) = <[u8; NONCE_LEN]>::try_read_from_prefix(request_slice)
            .map_err(|_| Error::MalformedRequest)?;

        Ok(Self { plaintext, id, nonce, aad, scheme: PhantomData })
    }
}
...
impl<'a, Scheme, const NONCE_LEN: usize, const TAG_LEN: usize> AeadDecryptRequest<'a, Scheme, NONCE_LEN, TAG_LEN> {
    pub fn new(id: ID, ciphertext: &'a [u8], tag: [u8; TAG_LEN], nonce:[u8; NONCE_LEN], aad: &'a [u8]) -> Self {
        Self { tag, ciphertext, id, nonce, aad, scheme: PhantomData }
    }
}
...
impl<'a, Scheme, const NONCE_LEN: usize, const TAG_LEN: usize> TryFrom<&'a mut [u8]> for AeadDecryptRequest<'a, Scheme, NONCE_LEN, TAG_LEN> {
    ...
    fn try_from(request: &'a mut [u8]) -> Result<Self, Self::Error> {
        let (tag, request_slice)  = <[u8; TAG_LEN]>::try_read_from_prefix(request)
            .map_err(|_| Error::MalformedRequest)?;
        ...
        Ok(Self { tag, ciphertext, id, nonce, aad, scheme: PhantomData })
    }
}

impl<'a, Scheme> From<&'a [u8]> for AeadDecryptResponse<'a, Scheme> {
    fn from(response: &'a [u8]) -> Self {
        Self { plaintext: response, scheme: PhantomData }
    }
}
