...
pub trait AEADKey<const KEY_SIZE: usize>: Send + Sync + From<[u8; KEY_SIZE]>{
    ...
    fn encrypt_msg<'a>(&self, ct: &'a mut [u8], nonce: Self::Nonce, aad: &[u8], plaintext: &[u8]) -> Result<(&'a [u8], Self::Tag), Error>;

    fn decrypt_msg<'a>(&self, pt: &'a mut [u8], nonce: Self::Nonce, aad: &[u8], ciphertext: &[u8], tag: Self::Tag) -> Result<&'a[u8], Error>;
}
...
fn map_libcrux_decrypt_error(err: libcrux_traits::aead::arrayref::DecryptError) -> Error {
    match err {
        ...
        libcrux_traits::aead::arrayref::DecryptError::Unknown => Error::Internal("Unknown Error".to_string()),
        _ => Error::Decrypt,
    }
}
